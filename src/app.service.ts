...
export class AppService {
  ...
  async sendEmail(userId: number, name: string, email: string): Promise<void> {
    await this.sendGrid
      .send({
        to: email,
        from: process.env.FROM_EMAIL,
        subject: 'User Created',
        text: `Hello ${name}, your user was created with success!`,
        html: `<strong>Hello ${name}, your user was created with success!</strong>`,
      })
      .then(async (response) => {
  ...
  async sendSMS(userId: number, name: string, phone: string): Promise<void> {
    const sms = this.client.getChannel('sms');

    const content = new TextContent(
      `Hello ${name}, your user was created with success!`,
    );
